...
public class ViewDragHelper {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    for (int i = 0; i < pointerCount; i++) {
                        ...
                        final float dx = x - mInitialMotionX[pointerId];
                        //final float dy = y - mInitialMotionY[pointerId];
                        final int dy = 0; // 为避免有时滑动错位的bug，并且由于酷我用到的滑动都是左右，强制改成上下不会移�?                        // 若以后再有上下滑动的�?��，可以改成上方注释掉的代�?	by huqian
    ...
    private boolean checkTouchSlop(View child, float dx, float dy) {
        ...
        // 限定只能横向滑动，角度小�?0度能触发swipe行为 byhuqian
        if (checkHorizontal && checkVertical) {
            return dx * dx + dy * dy > mTouchSlop * mTouchSlop;
        } else if (checkHorizontal) {
            return (Math.abs(dx) > mTouchSlop)&& Math.abs(dx)>Math.abs(dy)*2;
        } else if (checkVertical) {
            return (Math.abs(dy) > mTouchSlop)&& Math.abs(dy)>Math.abs(dx)*2;
        }
        return false;
    }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        final boolean offsetLeftAndRight = dx != 0 && Math.abs(dx)>Math.abs(dy)*2;
        final boolean offsetTopAndBottom = dy != 0 && Math.abs(dy)>Math.abs(dx)*2;
        if (offsetLeftAndRight) {
            clampedX = mCallback.clampViewPositionHorizontal(mCapturedView, left, dx);
            mCapturedView.offsetLeftAndRight(clampedX - oldLeft);
        }
        if (offsetTopAndBottom) {
